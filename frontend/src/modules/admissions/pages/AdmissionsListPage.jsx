...
const AdmissionsListPage = () => {
  ...
  const loadAdmissions = async () => {
    setLoading(true)
    try {
      const data = await admissionsService.getAdmissions(filters)
      ...
      <div style={styles.container}>
        <div style={styles.header}>
          <div style={{ display: 'flex', alignItems: 'center', gap: theme.spacing[3] }}>
            <button style={styles.backButton} onClick={() => navigate('/admissions')}>
              ← Back
            </button>
            <h1 style={styles.title}>Admissions</h1>
          </div>
          <button style={styles.addButton} onClick={() => navigate('/admissions/new')}>
            + New Admission
          </button>
        </div>
